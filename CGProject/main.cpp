...
int notForbidden(double x, double z)
{
    for (int i = 0; i < 5; i++)
    {
        if (x <= forbiddenlist[i].x1 && x >= forbiddenlist[i].x2 && z <= forbiddenlist[i].z1 &&
            z >= forbiddenlist[i].z2)
        {
            cout << "----" << i << " " << endl;
...
{
    p.data = new unsigned char[abs(
            p.infoHeader.biHeight * ((p.infoHeader.biWidth + 31) / 32 * 4) * p.infoHeader.biBitCount)];
    myfin.read((char *) p.data, sizeof(unsigned char) *
                                abs(p.infoHeader.biHeight * ((p.infoHeader.biWidth + 31) / 32 * 4) *
                                    p.infoHeader.biBitCount));
    cout << "ok";
    char swap;
    for (int i = 0;
         i < abs(p.infoHeader.biHeight * ((p.infoHeader.biWidth + 31) / 32 * 4) * p.infoHeader.biBitCount) / 3; i++)
    {
        swap = p.data[i * 3 + 0];
...
void loadTexture(int i, char *filename, bool type)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, bitmapInfoHeader.biWidth, bitmapInfoHeader.biHeight, 0, GL_RGB,
...
{
    glGenTextures(textureObjectCnt,
                  textureObjects);        // µÚÒ»²ÎÊýÊÇÐèÒªÉú³É±êÊ¾·ûµÄ¸öÊý, µÚ¶þ²ÎÊýÊÇ·µ»Ø±êÊ¾·ûµÄÊý×é
...
GLvoid draw_pointer()
{
    ...
    glBegin(GL_QUADS);
    glVertex3f(0, 0, 0.1);
    glVertex3f(0.5 * sin(((hour + min / 60 + sec / 3600) * 6 + 5) / 180 * M_PI),
               0.5 * cos(((hour + min / 60 + sec / 3600) * 6 + 5) / 180 * M_PI), 0.03);
    glVertex3f(0.6 * sin((hour + min / 60 + sec / 3600) * 6 / 180 * M_PI),
               0.6 * cos((hour + min / 60 + sec / 3600) * 6 / 180 * M_PI), 0.03);
    glVertex3f(0.5 * sin(((hour + min / 60 + sec / 3600) * 6 - 5) / 180 * M_PI),
               0.5 * cos(((hour + min / 60 + sec / 3600) * 6 - 5) / 180 * M_PI), 0.03);
    glEnd();
    ...
    glBegin(GL_QUADS);
    glVertex3f(0, 0, 0.01);
    glVertex3f(0.6 * sin(((min + sec / 60) * 6 + 2) / 180 * M_PI), 0.6 * cos(((min + sec / 60) * 6 + 2) / 180 * M_PI),
               0.02);
...
void key(unsigned char k, int x, int y)
{
    ...
    switch (k)
    {
        ...
        case '6':
        {
            ifmodelshow = !ifmodelshow;
            break;
        }
        case '7':
        {
            ifmodelscale = !ifmodelscale;
            break;
        }
        case '8':
        {
            ifmodeltranslate = !ifmodeltranslate;
            break;
        }
        case '9':
        {
            ifmodelrotate = !ifmodelrotate;
            break;
        }

        case ',':
        {
            ifdooropen = !ifdooropen;
            break;
        }
        case 27:
        case 'q':
